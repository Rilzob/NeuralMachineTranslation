...
def load_data(in_file):
    ...
    with open(in_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            en.append(['BOS'] + nltk.work_tokenize(line[0]) + ['EOS'])
            # 把中文句子分解为每个字
...
def encode(en_sentences, cn_sentences, en_dict, cn_dict, sort_by_len=True):
    ...
    # sort sentences by english length
    def len_argsort(seq):
...
def get_minibatches(n, minibatch_size, shuffle=False):
    ...
    minibatches = []
    for idx in idx_list:
        minibatches.append(np.array(np.arange(idx, min(idx + minibatch_size, n))))
    return minibatches


def prepare_data(seqs):
    # convert minibatch of seqs into numpy matrix
    lengths = [len(seq) for seq in seqs]
    n_samples = len(seqs)
    max_len = np.max(lengths)

    x = np.zeros((n_samples, max_len)).astype('int32')
    x_mask = np.zeros((n_samples, max_len)).astype('float32')  # indication of padding
